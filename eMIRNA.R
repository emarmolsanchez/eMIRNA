...
eMIRNA.Features <- function(file, prefix){
  ...
  GUp <- GUc1 + UGc1
    ...
    Sequence2 <- strsplit(Sequence, "")
    SecondaryStrc2 <- strsplit(SecondaryStrc, "")
    Matches5 <- lapply(SecondaryStrc2, function(x) which(x == "("))
    Matches3 <- lapply(SecondaryStrc2, function(x) which(x == ")"))
    Extract5 <- mapply('[', Sequence2, Matches5)
    Extract5 <- apply(Extract5,2,list)
    Extract5 <- lapply(Extract5, unlist)
    Extract3 <- mapply('[', Sequence2, Matches3)
    Extract3 <- apply(Extract3,2,list)
    Extract3 <- lapply(Extract3, unlist)
