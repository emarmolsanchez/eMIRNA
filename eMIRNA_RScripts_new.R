...
eMIRNA.Structural.Pvalue <- function(file, prefix, iterate=100, threshold=0.1, filter=TRUE){
  suppressMessages(require(Biobase))
  setwd("~/")
  Dir0 <- "eMIRNA"
  dir.create(file.path(Dir0), showWarnings=FALSE)
  ...
  final_table <- paste0(outdir, prefix, "_Structural_Pvalues.txt")
  ...
  if(filter == TRUE){
    ...
    name <- paste0(prefix, "_filtered_Pval.fa")
